...
async function startServer() {
  ...
  // Canvas Proxy (handles .ics feed and API calls without CORS issues)
...
Return only JSON, no markdown formatting.`;
      ...
      const fallbackAlerts = (req.body.emails || []).map((e: any) => {
        ...
        const isSpamKeywords = /khuyến mãi|voucher|giảm giá|ưu đãi|shopee|tiki|lazada|sale\s*\d+%|off\s*\d+%|quảng cáo|discount|coupon|unsubscribe|marketing|bản tin|deal|cashback/i.test(fullText);
        const isExam = /thi|kiểm tra|exam|quiz|test|midterm|final/i.test(fullText);
        const isAssignment = /bài tập|assignment|homework|lab|report|nộp bài|deadline|hạn nộp|rubric/i.test(fullText);
        ...
        return {
          ...
          categoryLabel,
          isSpam,
          spamReason: isSpam ? (isVietnamese ? "Thư quảng cáo / Khuyến mãi" : "Commercial promotion") : "",
  ...
  app.post("/api/gemini/quick-draft", async (req, res) => {
    try {
      const { teacherName, course, topic, draftType, studentNotes, tone, studentName, language } = req.body;
      const ai = getGenAI();
...
- Draft Intent / Type: ${draftType || "Clarification"} (e.g. Request Extension / Xin gia hạn nộp bài, Clarify Question / Giải đáp thắc mắc, Office Hours / Hẹn gặp trao đổi, Grade Inquiry / Thắc mắc điểm số, General Question)
...
${JSON.stringify(context || {}, null, 2)}`;
      ...
      if (formattedContents.length === 0) {
        formattedContents.push({ role: "user", parts: [{ text: "Hello! How can you help me today?" }] });
      }
      ...
      res.json({ reply: response.text || "I'm here to help you organize and conquer your school tasks!" });
    } catch (err: any) {
      console.error("Study assistant error:", err);
      res.json({
        reply: "I am ready to help you plan your study blocks, organize your assignments, and draft emails to your teachers!",
      });
    }
  });
