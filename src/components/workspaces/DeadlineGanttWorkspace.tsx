...
export const DeadlineGanttWorkspace: React.FC<{ assignments: Assignment[]; canvasAssignments: CanvasAssignment[] }> = ({ assignments, canvasAssignments }) => {
  ...
  // Scan ALL dated assignments: live props first, then the tracker + synced
  // caches the app already persists (read-only — a corrupt cache never blanks
  // the view). Dedup by key keeps the fallback additive, never duplicative.
  ...
  // In-app HTML/CSS timeline: one row per dated item grouped by course lane,
  // window padded to always include today so the today-line never vanishes.
