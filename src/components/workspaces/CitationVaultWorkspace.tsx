  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="bg-white dark:bg-[#1A1917] rounded-3xl border border-[#DFDACB] dark:border-[#2C2B27] p-6 shadow-card">
        <h2 className="text-lg font-bold flex items-center gap-2"><BookMarked className="w-5 h-5 text-[#D97757]" /> {t('cite_title')}</h2>
        <p className="text-xs text-[#6B6860] mt-1">{t('cite_sub')}</p>
        <div className="mt-4 flex gap-2">
          ...
          <button onClick={handleSearch} disabled={loading} className="px-4 py-2 bg-[#D97757] hover:bg-[#C86646] text-white rounded-xl text-xs font-bold flex items-center gap-1">{loading?t('cite_searching'):<><Search className="w-3.5 h-3.5" /> {t('cite_generate')}</>}</button>
        </div>
      </div>
      {result && (
        <div className="bg-white dark:bg-[#1A1917] rounded-3xl border border-[#DFDACB] dark:border-[#2C2B27] p-6 space-y-3">
          ...
          {([{label:'APA 7th', text: result.apa, key:'apa'}, {label:'MLA 9th', text: result.mla, key:'mla'}, {label:'Chicago 17th', text: result.chicago, key:'chi'}, {label:'BibTeX', text: result.bibtex, key:'bib'}] as const).map(f=>(
            <div key={f.key} className="p-3 bg-[#FAF9F5] dark:bg-[#1F1E1B] rounded-xl border border-[#DFDACB] dark:border-[#2C2B27]">
              <div className="flex items-center justify-between mb-1"><span className="text-xs font-bold">{f.label}</span><button onClick={()=>copy(f.text, f.key)} className="px-2 py-1 text-xs bg-white dark:bg-[#252422] border border-[#DFDACB] dark:border-[#2C2B27] rounded-lg flex items-center gap-1">{copied===f.key?<><Check className="w-3 h-3 text-emerald-600" /> {t('copied')}</>:<><Copy className="w-3 h-3" /> {t('copy')}</>}</button></div>
