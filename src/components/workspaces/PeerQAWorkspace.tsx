...
export const PeerQAWorkspace: React.FC = () => {
  ...
  const ask = () => {
    ...
    if (bad) { alert(t('pqa_blocked')); return; }
  ...
  return (
    <div className="space-y-4 max-w-4xl mx-auto">
      <div className="bg-white dark:bg-[#1A1917] rounded-3xl border border-[#DFDACB] dark:border-[#2C2B27] p-6 shadow-card">
        <h2 className="text-lg font-bold flex items-center gap-2"><MessageSquare className="w-5 h-5 text-[#D97757]" /> {t('pqa_title')}</h2>
        <p className="text-xs text-[#6B6860]">{t('pqa_sub')}</p>
        <div className="mt-4 space-y-2">
          <input value={course} onChange={e=>setCourse(e.target.value)} placeholder={t('pqa_course_ph')} className="w-full px-3 py-2 text-sm bg-[#FAF9F5] dark:bg-[#1F1E1B] border border-[#DFDACB] dark:border-[#2C2B27] rounded-xl" />
          <div className="flex gap-2">
            <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&ask()} placeholder={t('pqa_ask_ph')} className="flex-1 px-3 py-2 text-sm bg-[#FAF9F5] dark:bg-[#1F1E1B] border border-[#DFDACB] dark:border-[#2C2B27] rounded-xl" />
            <button onClick={ask} className="px-4 py-2 bg-[#D97757] hover:bg-[#C86646] text-white rounded-xl text-xs font-bold flex items-center gap-1"><Send className="w-3.5 h-3.5" /> {t('pqa_ask')}</button>
          </div>
        </div>
      </div>
      <div className="space-y-2">
        {qs.slice().reverse().map(q=>(
          <div key={q.id} className="p-4 bg-white dark:bg-[#1A1917] rounded-2xl border border-[#DFDACB] dark:border-[#2C2B27]">
            <div className="flex items-center gap-2 text-xs font-bold"><span className="px-2 py-0.5 rounded bg-amber-100 text-amber-800 text-[10px]">{q.course}</span> {t('pqa_anon')} <ShieldCheck className="w-3 h-3 text-emerald-600" /></div>
            ...
            <div className="mt-2 flex gap-2">
              <input value={drafts[q.id] || ''} onChange={e=>setDrafts(d=>({...d, [q.id]:e.target.value}))} onKeyDown={e=>e.key==='Enter'&&reply(q.id)} placeholder={t('pqa_reply_ph')} className="flex-1 px-3 py-1.5 text-xs bg-[#FAF9F5] dark:bg-[#1F1E1B] border border-[#DFDACB] dark:border-[#2C2B27] rounded-xl" />
              <button onClick={()=>reply(q.id)} className="px-3 py-1.5 bg-[#D97757] hover:bg-[#C86646] text-white rounded-xl text-xs font-bold">{t('pqa_reply')}</button>
            </div>
          </div>
        ))}
        {qs.length===0 && <p className="text-xs text-[#6B6860] text-center py-8">{t('pqa_empty')}</p>}
      </div>
    </div>
  );
};
