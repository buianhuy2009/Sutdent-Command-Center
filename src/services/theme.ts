...
export function getTheme(): ThemeId {
  try { const s = localStorage.getItem(THEME_KEY) as ThemeId | null; return (s as ThemeId) || 'linen'; } catch { return 'linen'; }
}
export function setTheme(id: ThemeId) {
  ...
  if (isDarkTheme) {
    ...
    // respect scc_theme for linen
    try {
      const dark = localStorage.getItem('scc_theme')==='dark';
...
export function initTheme() {
  ...
  if (isDarkTheme) document.documentElement.classList.add('dark');
    ...
    try {
      ...
      const shouldDark = s ? s==='dark' : prefersDark;
...
export function syncDarkToTheme(isDark: boolean) {
  ...
  if (isDarkTheme && !isDark) {
    // user toggled light while on dark theme -> switch to linen
    setTheme('linen');
    document.documentElement.classList.remove('dark');
  }
  if (isDark) document.documentElement.classList.add('dark');
  else if (!isDarkTheme) document.documentElement.classList.remove('dark');
